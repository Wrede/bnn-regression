...
def MCinferMOG(data, model, num_monte_carlo):
    
        m = np.empty((num_monte_carlo,1,2))
        S = np.empty((num_monte_carlo,1,2,2))
        ...
        matmul = np.empty((num_monte_carlo, 1, 2, 2))
        for i in range(num_monte_carlo):
            inner = m[i] - m_hat
...
class Regressor():
    ...
    def train(self, batch_size=32, epochs=400):
        ...
        model = self.construct_small(input_dim, output_dim, len(self.train_ts))
        model.summary()
    ...
    def run(self, target, batch_size=32, num_monte_carlo=500, epochs=100, retrain=False, infer_pdf=True, verbose=True):
        ...
        if retrain:
            ...
            self.model = self.train(batch_size=batch_size, epochs = epochs)
        if infer_pdf:
                self.proposal_mean, self.proposal_covar = MCinferMOG(target, self.model, num_monte_carlo)  
        else:
